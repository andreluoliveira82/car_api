...
def validate_brand_name(value: str) -> str:
    ...
    if not re.match(r'^[A-Za-z0-9\s\-]+$', value):
        raise ValueError(
            'O nome da marca deve conter apenas letras, números, espaços e hífens.'
        )
...
def validate_brand_description(value: str) -> str:
    if len(value) > MAX_BRAND_DESCRIPTION:
        raise ValueError(
            f'A descrição deve ter no máximo {MAX_BRAND_DESCRIPTION} caracteres.'
        )
    return value.strip()
...
def validate_car_factory_year(value: int) -> int:
    ...
    if value > current_year + MAX_FUTURE_YEAR:
        raise ValueError(
            f'O ano de fabricação não pode ultrapassar {current_year + MAX_FUTURE_YEAR}.'
        )
...
def validate_car_model_year(value: int, factory_year: int) -> int:
    ...
    if value > current_year + MAX_FUTURE_YEAR:
        raise ValueError(
            f'O ano do modelo não pode ultrapassar {current_year + MAX_FUTURE_YEAR}.'
        )
...
def validate_car_plate(value: str) -> str:
    ...
    if not (re.match(old_pattern, value) or re.match(mercosul_pattern, value)):
        raise ValueError(
            'Placa inválida. Aceitos: padrão antigo (AAA0000) ou Mercosul (AAA0A00).'
        )
